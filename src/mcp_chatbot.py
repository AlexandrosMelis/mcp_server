...
class MCP_ChatBot:
    ...
    async def connect_to_server(self, server_name, server_config):
        try:
            ...
            try:
                ...
                print(f"Error initializing capabilities for server '{server_name}': {e}")
                traceback.print_exc()
    ...
    async def process_query(self, query):
        ...
        while True:
            ...
            print(f"AI message received: {ai_message}")

            if not ai_message:
                raise Exception("LLM did not return a response")

            # convert chatbot pipeline to langchain compatible
            if isinstance(ai_message.content, str) and len(ai_message.content) > 0:
                print(f"Assistant response content: {ai_message.content}")
                messages.append(ai_message)
            elif ai_message.tool_calls:

                tool_messages = []

                print(f"tools: {[tool_call['name'] for tool_call in ai_message.tool_calls]}")
                has_tool_use = True
                messages.append(ai_message)
                ...
                for tool_call in ai_message.tool_calls:
                    ...
                    # parse result (may contain multiple contents)
                    print(f"\n\n####### Tool result:\n{result}\n\n")
